...
## Security
قطعاً یکی از اهداف حمله، بازپخش انتقالات مبتنی بر VoIP است. برای دستیابی به این هدف، مهاجمان باید به جریان RTP دسترسی پیدا کنند. متأسفانه، همان مکانیزمی که به گیرنده اجازه می‌دهد تا روش رمزگذاری استفاده شده توسط فرستنده را درک کند، به مهاجم نیز اجازه می‌دهد تا جریان را رمزگشایی کند. هر بسته شامل شناسه payload است که کدک را شناسایی می‌کند. شناسه منبع همگام‌سازی (synchronization source identifier) به گیرنده (و مهاجم) اجازه می‌دهد تا بسته‌ها را از همان منبع جمع‌آوری کند و شماره‌های ترتیبی (sequence numbers) آن‌ها را به ترتیب نگه دارد.

پس از جمع‌آوری و مرتب‌سازی بسته‌ها، بازپخش آن‌ها کار ساده‌ای است. شکل 4-25 پلیر (player) داخلی Wireshark را نمایش می‌دهد. این پلیر پس از انتخاب یک بسته RTP و انجام تحلیل جریان RTP از منوی Telephony فعال می‌شود.

## Vectors
مهاجم چگونه به جریان RTP دسترسی پیدا می‌کند؟ گسترش شبکه‌های بی‌سیم همچنین منجر به گسترش نقاط پایانی بی‌سیم مانند تلفن‌ها می‌شود. حمله به یک شبکه بی‌سیم ساده است: ترافیک را ضبط کنید. همان ابزارهایی که یک پلیر (player) را فراهم می‌کنند، همچنین قادر به ضبط فریم‌های بی‌سیم هستند. اما حتی بدون دسترسی به شبکه بی‌سیم یا اگر شبکه بی‌سیم رمزگذاری شده باشد، مهاجم می‌تواند با حمله به دستگاه‌های زیرساختی، به جریان‌های RTP دسترسی پیدا کند. دو روش محبوب عبارتند از پر کردن بیش از حد جدول آدرس منبع در یک سوئیچ و جعل یک پورت ترانک در یک سوئیچ.

در حمله به جدول آدرس منبع (که به عنوان پر کردن جدول آدرس MAC نیز شناخته می‌شود)، حافظه سوئیچ به طور مداوم با آدرس‌های MAC پر می‌شود به طوری که آدرس‌های معتبر نمی‌توانند به جدول اضافه شوند. ترافیک مقصد به این آدرس‌های MAC معتبر باید از همه پورت‌ها عبور کند. جعل یک پورت ترانک حمله‌ای است که در آن سوئیچ هدف فریب داده می‌شود تا باور کند که یک خط ترانک متصل است. ترافیک مقصد به آدرس‌های MAC ناشناخته مانند ترافیک پخش‌شده (broadcast) از پورت‌های ترانک عبور داده می‌شود. مهاجم همچنین می‌تواند با برچسب‌گذاری ترافیک و پرش VLAN، ترافیک را به مقاصد خاصی ارسال کند. حملات علیه میزبان‌ها می‌توانند آن‌ها را فریب دهند تا ترافیک را به مهاجم ارسال کنند یا اجازه دهند مهاجم به عنوان یک مرد میانی عمل کند.

در مواجهه با این چالش‌ها، جریان‌های RTP باید رمزگذاری شوند تا حریم خصوصی آن‌ها محافظت شود. RFC 3711 پروتکل Secure Real-Time Transport (یا SRTP) را توضیح می‌دهد. RFC 3711 همچنین SRTCP را تعریف می‌کند و بنابراین امکاناتی برای حریم خصوصی و تأیید اعتبار هر دو پیام RTP و RTCP فراهم می‌کند.

## SRTP Operation
در RTP امن (Secure RTP) به عنوان یک پروفایل از RTP در نظر گرفته می‌شود. بنابراین، SRTP برای اهداف خود، تغییرات جزئی در RTP ایجاد می‌کند. SRTP و SRTCP از همان ساختار RTP و RTCP استفاده می‌کنند، با این تفاوت که اطلاعاتی برای انجام وظایف اضافی اضافه شده است. شکل 4-26 ساختار بسته (packet) را نشان می‌دهد.

![](../image/4-26.png)

Here's the translation:

بسته SRTP کل هدر RTP را به اشتراک می‌گذارد و فیلدهای اشاره شده توسط فلش را اضافه می‌کند. همچنین می‌توانیم ببینیم که در حالی که کل بسته تأیید اعتبار می‌شود، تنها بخشی از آن رمزگذاری شده است. فیلدهای SRTP شامل موارد زیر است:

- **شاخص کلید اصلی (Master Key Index)**: این یک فیلد اختیاری است که می‌تواند برای ارائه اطلاعاتی در مورد اینکه کدام کلید اصلی باید استفاده شود، مورد استفاده قرار گیرد.
- **برچسب تأیید اعتبار (Authentication Tag)**: این فیلد برای ارائه مقداری که توسط الگوریتم تأیید اعتبار محاسبه شده است، استفاده می‌شود.

RFC 3711 کلیدها و الگوریتم‌های پیش‌فرض (اگرچه دیگران نیز پشتیبانی می‌شوند) برای استفاده در یک پیاده‌سازی امن را تعریف کرده است. برای رمزگذاری، "رمز پیش‌فرض" استاندارد رمزگذاری پیشرفته، یا AES است. برای تأیید اعتبار، کد تأیید اعتبار پیام هش‌شده – الگوریتم هش ایمن 1 (HMAC-SHA1) مشخص شده است. دو کلید استفاده می‌شوند: یک کلید اصلی و یک کلید جلسه. نقاط پایانی و سرور تماس از کلید اصلی برای استخراج کلید جلسه استفاده می‌کنند. کلیدهای جلسه در واقع برای رمزگذاری داده‌های صوتی یا تصویری استفاده می‌شوند. RFC توزیع کلید را مشخص نمی‌کند. این موضوع اغلب توسط پروتکل سیگنالینگ مدیریت می‌شود.

ایده این است که نقاط پایانی چیزی به نام "زمینه رمزنگاری" (cryptographic context) را نگه می‌دارند که اطلاعاتی درباره الگوریتم‌ها، کلیدها و وضعیت فعلی اتصال است. اما نقاط پایانی همچنین شمارنده‌های چرخشی (که تعداد استفاده مجدد از شماره‌های ترتیبی را شمارش می‌کنند)، لیست‌های بازپخش، و هر کلید نمکی را دنبال می‌کنند. افزودن نمک (Salting) به فرآیند تولید جلسه، ماده اضافی اضافه می‌کند تا استخراج کلید جلسه از خارج دشوارتر شود. SRTP و SRTCP کلید اصلی و زمینه رمزنگاری یکسانی را به اشتراک می‌گذارند اما معمولاً از کلیدهای جلسه یکسان استفاده نمی‌کنند. شکل 4-27 ساختار SRTCP توصیف شده توسط RFC 3711 را نشان می‌دهد. باز هم می‌توانیم ساختار مشترک بسته را ببینیم. کل پیام تأیید اعتبار می‌شود، اما فقط داده‌های مربوط به جریان رمزگذاری شده است. فیلدهای MKI و برچسب تأیید اعتبار همان هدفی را دارند که در SRTP داشتند.

![](../image/4-27.png)


همه تولیدکنندگان از SRTP و SRTCP پشتیبانی نمی‌کنند. حتی برای آن‌هایی که پشتیبانی می‌کنند، اغلب در یک محیط بالغ، دستگاه‌های پایانی سطوح مختلفی از پشتیبانی از ویژگی‌ها یا الزامات رمزگذاری دارند. در نهایت، برخی از این موارد بین تولیدکنندگان متفاوت است. اگر می‌خواهید انتقال‌های رسانه‌ای را در جریان‌های زمان واقعی رمزگذاری کنید، باید به دقت برنامه‌ریزی پیاده‌سازی مورد نظر را با توجه به پروفایل SRTP بررسی کنید.

می‌توان این استدلال را مطرح کرد که رمزگذاری ضروری نیست زیرا اکثر نقاط پایانی VoIP سیمی و داخلی هستند. این موضوع به طور ضروری یک تصمیم محلی است، اما حضور شبکه‌های بی‌سیم، راه‌حل‌های میزبانی شده، دسترسی مهمان، کار از راه دور، یا سایر شرایطی که در آن‌ها جریان‌های RTP ممکن است در معرض خطر قرار بگیرند، نیاز به بررسی دقیق مشخصات شبکه را نشان می‌دهد.


---------------

در RFC 3711 یک استاندارد برای رمزگذاری و احراز هویت داده‌های صوتی و تصویری در شبکه‌ها است. این استاندارد از کلیدها و الگوریتم‌های از پیش تعریف‌شده‌ای استفاده می‌کند که البته قابلیت استفاده از دیگر الگوریتم‌ها نیز وجود دارد. برای رمزگذاری، الگوریتم پیش‌فرض "AES" یا همان استاندارد رمزگذاری پیشرفته است. برای احراز هویت، از "HMAC-SHA1" استفاده می‌شود که یک کد احراز هویت پیام هش شده با استفاده از الگوریتم "SHA1" است.

در این استاندارد، دو نوع کلید مورد استفاده قرار می‌گیرد: کلید اصلی (Master Key) و کلید نشست (Session Key). کلید اصلی توسط نقاط انتهایی (endpoints) و سرور تماس برای تولید کلید نشست مورد استفاده قرار می‌گیرد. کلیدهای نشست در واقع همان کلیدهایی هستند که برای رمزگذاری داده‌های صوتی یا تصویری استفاده می‌شوند. این RFC نحوه توزیع کلیدها را مشخص نمی‌کند و معمولاً این کار توسط پروتکل‌های سیگنالینگ انجام می‌شود.

ایده اصلی این است که نقاط انتهایی اطلاعات مربوط به "بافت رمزنگاری" (Cryptographic Context) را حفظ کنند. این اطلاعات شامل الگوریتم‌ها، کلیدها و وضعیت فعلی ارتباط است. همچنین، نقاط انتهایی باید شمارنده‌های چرخه (rollover counters) که تعداد استفاده مجدد از شماره‌های ترتیبی (sequence numbers) را شمارش می‌کنند، لیست‌های بازپخش (replay lists)، و کلیدهای نمک (salt keys) را نیز ردیابی کنند. استفاده از نمک به فرایند تولید کلید نشست مواد اضافی اضافه می‌کند تا کلید نشست برای افراد خارجی دشوارتر قابل استخراج باشد.

در SRTP و SRTCP از کلید اصلی و بافت رمزنگاری مشترکی استفاده می‌کنند، اما معمولاً از کلیدهای نشست متفاوتی استفاده می‌کنند. در ساختار بسته SRTCP که توسط RFC 3711 توصیف شده، کل پیام احراز هویت شده، اما تنها داده‌های مربوط به جریان رمزگذاری می‌شود. فیلدهای MKI و برچسب احراز هویت همان کاربردی را دارند که در SRTP نیز داشتند.

[SRTP and You: A Deep Dive into Encrypted VoIP Communications](https://www.twilio.com/en-us/blog/srtp-deep-dive)

[TLS and SRTP](https://support.telnyx.com/en/articles/4404575-tls-and-srtp)

[SRTP in VoIP telephony: How to secure calls against eavesdropping](https://www.tenios.de/en/blog/srtp-in-voip-telephony)

# Chapter 5
جریان‌های صوتی و تصویری مانند آن‌هایی که هنگام صحبت در یک میکروفون کامپیوتر یا استفاده از دوربین دیجیتال ایجاد می‌شوند، ابتدا به صورت سیگنال‌های آنالوگ شروع می‌شوند. زمانی بود که سیگنال صوتی به طور کامل در شبکه تلفن به همین شکل منتقل می‌شد. با این حال، این معماری محدود و پیچیده است. امروزه، مکالمات صوتی یا تصویری، چه در یک سیستم تلفنی سنتی و چه در سیستمی مبتنی بر VoIP (Voice over IP)، ابتدا به صورت آنالوگ شروع می‌شوند اما سپس به فرمت دیجیتال تبدیل می‌شوند. نمایه دیجیتال صدا یا تصویر به گیرنده منتقل شده و سپس دوباره به آنالوگ تبدیل می‌شود تا توسط انسان در انتهای دیگر درک شود. به طور کلی، این فرآیند تبدیل آنالوگ به دیجیتال و دیجیتال به آنالوگ توسط کدک (Codec) یا رمزگذار-رمزگشا انجام می‌شود.

تکنیک‌های مختلفی برای مدیریت این جریان‌های صوتی و تصویری استفاده می‌شود. بیشتر کدک‌های محبوب که امروزه استفاده می‌شوند، در توصیه‌های ITU-T (اتحادیه بین‌المللی مخابرات-تلکام) استاندارد شده‌اند، هرچند که بسیاری دیگر نیز وجود دارند. بیشتر کارهایی که با کدک‌ها انجام شده است، در جهت کاهش مقدار پهنای باند مصرفی توسط جریان صوتی از طریق استفاده از فشرده‌سازی بوده است. استقرار VoIP نیاز به انجام همان فرآیندهای تبدیل دارد، اگرچه همیشه نگرانی‌های مربوط به پهنای باند در توپولوژی‌های سنتی‌تر را ندارد. فهم کدک‌های رایج و عملکرد پایه‌ای آن‌ها می‌تواند به استقرار اولیه و رفع مشکلات کمک کند. در این فصل، فرکانس‌های ارتباطی و کدک‌های صوتی و تصویری رایج مورد بررسی قرار می‌گیرند. علاوه بر این، خواننده برخی از بینش‌ها درباره انتخاب کدک و کیفیت تماس را به دست خواهد آورد.

## Audio Frequencies
دلایل بسیار خوبی برای ساختاردهی شبکه تلفن به شکلی که در فصل 2 توضیح داده شده است وجود دارد. به عنوان انسان، ما در زمینه شنیدن و تولید صداها دارای قابلیت‌های محدودی هستیم. گوش انسان می‌تواند فرکانس‌هایی به پایین‌ترین حد 20 هرتز (Hz) و تا حدود 20,000 هرتز یا 20 کیلوهرتز (KHz) را بشنود. شما ممکن است آگاهی از محدوده شنوایی خود را بر اساس تست‌های شنوایی که در مدرسه انجام شده است، داشته باشید. محدوده‌های شنوایی برای انواع مختلف حیوانات، از جمله انسان‌ها، در شکل 5-1 نشان داده شده است. همانطور که می‌بینیم، در مقایسه با سایر حیوانات، ما انسان‌ها چیزی برای افتخار کردن نداریم.

![](../image/5-1.png)

ما همچنین می‌توانیم صداهایی تولید کنیم، اما دامنه تولید صداهای ما بسیار کمتر از دامنه‌ای است که می‌توانیم بشنویم. بر اساس کتاب رکوردهای جهانی گینس، پایین‌ترین فرکانسی که توسط یک انسان تولید شده، کمی بالاتر از 0 هرتز بوده است و بالاترین فرکانس کمی کمتر از 5000 هرتز بوده است. شکل 5-2 به ما نشان می‌دهد که در مقایسه با سایر پستانداران چگونه عمل می‌کنیم.

![](../image/5-2.png)

در حالی که کل محدوده برای گفتار انسان به‌طور کامل کاربردی نیست (احتمالاً مکالمه‌ای در فرکانس 5000 هرتز نخواهید داشت)، این محدوده مبنایی برای بحث ما درباره اندازه کانال صوتی در سیستم‌های تلفنی سنتی فراهم می‌کند. از آنجا که بیشتر صداهایی که ما تولید می‌کنیم دارای فرکانس‌های پایین هستند، اتصال محلی استاندارد به دامنه 0 تا 4000 هرتز اختصاص داده شده است. در سیستم‌های انتقال، این محدوده در واقع به 300 تا 3400 هرتز کاهش می‌یابد، همان‌طور که در اسنادی مانند توصیه ITU-T G.107 ذکر شده است. این توصیه مدل محاسباتی برای برنامه‌ریزی انتقال را ارائه می‌دهد.

اگر این محدوده فرکانس‌ها به هر مدار اختصاص داده شده باشد، هر سیگنال دیگری که از همان کانال استفاده می‌کند نیز باید در همین محدوده فرکانس‌ها قرار بگیرد. بنابراین، تمام صداها و تن‌هایی که در گوشی تلفن خود می‌شنوید در این محدوده قرار دارند. در واقع، فیلترهای باند-گذر از عبور فرکانس‌هایی خارج از این محدوده جلوگیری می‌کنند.

## Voice Signals
یک سیگنال صوتی یا صوتی یک موج آنالوگ است. یک موج آنالوگ پایه در شکل 5-3 نشان داده شده است. این تصویر یک نگاه سه‌ثانیه‌ای به یک صدای تولید شده با فرکانس 500 هرتز را ارائه می‌دهد. این موج به نظر می‌رسد که یک بلوک پیوسته از اطلاعات آنالوگ باشد. در فرکانس 500 هرتز، در هر ثانیه 500 چرخه یا موج کامل وجود دارد. یک موج واحد می‌تواند در یک پانصدم ثانیه، یا 0.002 ثانیه منتقل شود. این زمان همچنین به عنوان دوره موج شناخته می‌شود. به عبارت دیگر، در سه ثانیه‌ای که در شکل 5-3 مشاهده می‌شود، در واقع 1500 موج وجود دارد.


![](../image/5-3.png)

در شکل 5-4، با بزرگنمایی و بررسی آنچه که در یک صدم ثانیه (0.01 ثانیه) اتفاق می‌افتد، می‌توان موج را مشاهده کرد. در واقع، اگر از اولین نقطه عبور از صفر تا آخرین نقطه عبور از صفر اندازه‌گیری کنیم، پنج چرخه (سیکل) وجود دارد.

![](../image/5-4.png)

موج‌های آنالوگ دارای سه ویژگی اصلی هستند: فرکانس، دامنه (Amplitude)، و فاز. فاز (ارتباط زمانی یا نسبت به یک نقطه دیگر در زمان) در بحث ما نقش زیادی ندارد، اما دامنه (قدرت موج) و فرکانس (تعداد چرخه‌ها در هر ثانیه) اهمیت دارند. چون این موج یک صدای ثابت است که حجم (دامنه) آن تغییر نمی‌کند، فرکانس و دامنه در طول تولید صدا ثابت می‌مانند. فرکانس 500 هرتز در محدوده شنوایی انسان قرار دارد و در محدوده فرکانس‌هایی که در مدار تلفنی مجاز هستند نیز قرار می‌گیرد. اما گوش دادن به این صدا احتمالاً خسته‌کننده خواهد بود، زیرا هیچ تغییری ندارد؛ البته یک فایل MP3 از این صدا در وب‌سایت کتاب موجود است.

گفتار انسانی مجموعه‌ای از فرکانس‌هایی است که تغییر می‌کنند. حتی ساده‌ترین سیگنال نیز شامل طیف گسترده‌ای از این فرکانس‌هاست. یک نمونه از سیگنال صوتی در شکل 5-5 نشان داده شده است. در این مورد، موج آنالوگ چهار ثانیه‌ای نشان‌دهنده کلمه "Hello" است که سه بار گفته شده است.

![](../image/5-5.png)

اگرچه می‌توانیم مقداری تغییرات در سیگنال مشاهده کنیم حتی زمانی که همان کلمه تکرار می‌شود، اما آنچه از تصویر به وضوح مشخص نیست، این است که سیگنال چقدر می‌تواند پیچیده باشد. با گسترش پایه زمانی موج و تمرکز بر روی یک بخش کوچک‌تر، می‌توانیم برخی از جزئیات را ببینیم. شکل 5-6 یک چهارم ثانیه از اولین "Hello" را نشان می‌دهد.

![](../image/5-6.png)

برخلاف بخش گسترده‌شده از صدای 500 هرتز، در اینجا تغییراتی در فرکانس و دامنه مشاهده می‌شود. این به این دلیل است که صداهایی که ما تولید می‌کنیم، حتی در داخل یک کلمه مشابه، از نظر قدرت و تنوع تغییر می‌کنند. سیگنال‌هایی مانند این‌ها از طریق سیم‌های تلفن به کارت خط در سوئیچ یا مرکز مخابراتی ارسال می‌شوند. چالش برای یک کدک (Codec) این است که تمام این تغییرات را در یک سری نمونه‌ها (Samples) ثبت کند تا بتوان آن‌ها را در انتهای دیگر به‌طور معناداری دوباره بازسازی کرد.

In voice (or any sound), **frequency**, **amplitude**, and **phase** are key concepts that describe how the sound wave behaves. Let me explain each one in a simple way, along with examples related to voice.

### 1. **Frequency**
- **What it is**: Frequency is how fast the sound wave vibrates or repeats itself over time.
- **Measured in**: Hertz (Hz), which tells how many times the wave cycles (goes up and down) in one second.
- **What it affects**: Frequency controls the **pitch** of the sound.
  - Higher frequency = higher pitch (like a bird's chirp).
  - Lower frequency = lower pitch (like a man's deep voice).

**Example**: 
- A woman’s voice usually has a higher frequency than a man’s, so it sounds higher-pitched. A typical adult male voice may have a frequency around 100-150 Hz, while a woman’s voice is typically around 200-250 Hz.

### 2. **Amplitude**
- **What it is**: Amplitude refers to the height of the sound wave.
- **Measured in**: Decibels (dB), which shows how strong or loud the sound is.
- **What it affects**: Amplitude determines the **loudness** of the sound.
  - Larger amplitude = louder sound.
  - Smaller amplitude = softer sound.

**Example**: 
- If you whisper, your voice has a small amplitude (low volume). If you shout, your voice has a large amplitude (high volume).

### 3. **Phase**
- **What it is**: Phase describes the position of the sound wave at a given time. It tells us where the wave is starting.
- **Measured in**: Degrees (°), from 0° to 360°.
- **What it affects**: Phase doesn’t affect how we hear a single sound by itself much, but it can matter when two sound waves mix. If two waves are out of phase, they can cancel each other out.

**Example**: 
- If you record your voice on two microphones and they pick up the sound slightly differently in time, the phase might be different. If the waves are perfectly out of phase (180°), the sound could become quieter or even cancel out.

### Summary with Voice Example:
- **Frequency**: Controls if your voice sounds deep or high-pitched.
- **Amplitude**: Controls how loud or soft your voice is.
- **Phase**: Controls how waves interact when multiple sounds combine.

---------------

Let's break down the concepts even further with the help of the image and more detailed explanations:

### 1. **Frequency** (Top Section in the Image)
- **What you see**: Two waveforms, one with closely spaced peaks and the other with widely spaced peaks.
- **What it means**: 
  - **High frequency** (closely spaced peaks): This represents a higher pitch sound, like a soprano singer or a bird chirping. The wave cycles very quickly, so it repeats more times per second.
  - **Low frequency** (widely spaced peaks): This represents a lower pitch sound, like a deep male voice or a bass guitar note. The wave cycles more slowly, so it repeats fewer times per second.

#### Voice Example:
- When you sing a high note, the frequency of your vocal cords' vibration is high, producing closely spaced sound waves.
- When you speak in a deep voice, the frequency is low, producing waves that are more spread out.

### 2. **Amplitude** (Middle Section in the Image)
- **What you see**: Two waves with different heights. One has a larger height (amplitude), and the other has a smaller height.
- **What it means**: 
  - **High amplitude** (tall waves): This represents a louder sound. When you shout, the energy in your voice increases, and the wave becomes larger.
  - **Low amplitude** (short waves): This represents a softer sound, like when you're whispering. The wave is shorter because the energy is lower.

#### Voice Example:
- If you speak quietly, the amplitude is low because less energy is pushing the sound.
- If you yell, the amplitude is high because there’s more energy behind your voice.

### 3. **Phase** (Bottom Section in the Image)
- **What you see**: Two waves that have the same frequency and amplitude but start at different points. One starts at 0° (the beginning of the cycle) and the other at 180° (halfway through the cycle).
- **What it means**: 
  - **In-phase**: When two sound waves start at the same point, their peaks and valleys match up, which can make the sound louder.
  - **Out of phase**: When two waves are 180° out of phase, their peaks and valleys are opposite, which can cause the sounds to cancel each other out.

#### Voice Example:
- Imagine two people speaking at the same time. If their sound waves are "in phase," the sound might get louder. If they are "out of phase," the sounds might cancel each other, making it hard to hear either one.

### Conclusion:
- **Frequency** changes the **pitch** of the sound (high or low voice).
- **Amplitude** changes the **volume** (loud or soft voice).
- **Phase** affects how multiple sounds combine, which can make them louder or quieter depending on their alignment.

This breakdown should help you see how these elements interact to create the sound of your voice!

## Audio Coders and Decoders
کدک وظیفه دارد موج آنالوگ را به یک سری نمونه‌ها تبدیل کند و سپس یک مقدار باینری برای آن نمونه فراهم کند. این کار به این دلیل انجام می‌شود که داده‌های صوتی بتوانند از طریق بخش‌های دیجیتال شبکه تلفن عمومی سوئیچ شده (PSTN) منتقل شوند. اما این تنها نیمی از ماجراست. در گیرنده، این نمونه‌ها باید مجدداً مونتاژ شده و به یک موج آنالوگ تبدیل شوند که بتواند به درستی تفسیر شود. این فرآیند «مدولاسیون کد پالس» (PCM) نامیده می‌شود و به‌عنوان روش پایه‌ای که در PSTN و بسیاری از پیاده‌سازی‌های VoIP برای کدگذاری داده‌های صوتی استفاده می‌شود، عمل می‌کند. این روش در توصیه‌نامه G.711 اتحادیه بین‌المللی مخابرات (ITU-T) تعریف شده است. بیایید نگاهی دقیق‌تر به آن بیندازیم.

## Sampling
به خاطر داشته باشید که معمولاً هر دو سر اتصال آنالوگ هستند و موج آنالوگ ابتدا باید نمونه‌برداری شود. در مدولاسیون کد پالس (PCM)، یک نمونه‌گیری، در واقع ارزیابی قدرت سیگنال در زمان نمونه‌برداری است. در شکل 5-7 نسخه‌ای بزرگ‌تر از شکل 5-6 نمایش داده شده است. خطوط عمودی مکان‌ها یا زمان‌های نمونه‌های جداگانه هستند و نقاط مکان‌های موج سیگنال را نشان می‌دهند. نکته کلیدی در نمونه‌برداری این است که به اندازه کافی از نمونه‌ها گرفته شود تا بازتولید دقیقی از سیگنال اصلی ارائه شود، اما نه آنقدر زیاد که بیش از حد نمونه‌برداری شود. همان‌طور که از فلش‌ها در شکل 5-7 مشاهده می‌شود، نرخ نمونه‌برداری پایین می‌تواند باعث از دست رفتن جزئیات سیگنال اصلی شود. علاوه بر این، نرخ نمونه‌برداری پایین ممکن است باعث بروز پدیده‌ای به نام "علیاسینگ" شود. این اتفاق زمانی رخ می‌دهد که سیگنال منتقل‌شده به نظر می‌رسد فرکانسی پایین‌تر از سیگنال واقعی دارد. از سوی دیگر، نمونه‌برداری با فرکانس بیش از حد بالا منجر به مصرف غیرضروری پهنای باند می‌شود.

![](../image/5-7.png)

-----

در Sampling در صوت به معنای گرفتن نمونه‌هایی از یک موج صوتی در فواصل زمانی منظم است تا آن را به یک سیگنال دیجیتال تبدیل کنیم. به زبان ساده، وقتی می‌خواهیم صدا را در کامپیوتر ذخیره کنیم، به جای اینکه تمام موج آنالوگ را ضبط کنیم، تعداد زیادی نقطه یا نمونه از آن می‌گیریم. هرچه تعداد این نمونه‌ها بیشتر باشد (نرخ نمونه‌برداری بالاتر)، کیفیت صدا بهتر است و به صدای اصلی نزدیک‌تر می‌شود.

مثلاً اگر نرخ نمونه‌برداری 44.1 کیلوهرتز باشد، یعنی در هر ثانیه 44100 نمونه از موج صوتی گرفته می‌شود.

البته! برای درک بهتر مفهوم sampling در صوت، می‌توانیم از مثال‌های ساده استفاده کنیم:

### 1. **موج صوتی آنالوگ:**
تصور کن که یک موج صوتی واقعی مانند امواج دریا است. این موج پیوسته و بدون قطع و وصله است. وقتی می‌خواهیم این موج را به صورت دیجیتال ذخیره کنیم، باید آن را به قسمت‌های کوچک‌تر تقسیم کنیم.

### 2. **نرخ نمونه‌برداری (Sampling Rate):**
نرخ نمونه‌برداری نشان می‌دهد که چه تعداد نمونه از موج صوتی در هر ثانیه گرفته می‌شود. برای مثال:
- **44.1 کیلوهرتز:** در هر ثانیه 44,100 نمونه گرفته می‌شود. این نرخ معمولاً برای CD‌های صوتی استفاده می‌شود و کیفیت خوبی دارد.
- **48 کیلوهرتز:** در هر ثانیه 48,000 نمونه. این نرخ معمولاً در ویدئو و فیلم‌های دیجیتال استفاده می‌شود.

### 3. **عمق بیت (Bit Depth):**
عمق بیت نشان می‌دهد که هر نمونه از صدا با چه دقتی ذخیره می‌شود. برای مثال:
- **16 بیت:** هر نمونه به 65,536 مقدار مختلف تقسیم می‌شود. این معمولاً برای فایل‌های صوتی CD استفاده می‌شود.
- **24 بیت:** دقت بیشتری دارد و معمولاً در تولیدات حرفه‌ای استفاده می‌شود.

### 4. **تبدیل به دیجیتال:**
برای تبدیل موج آنالوگ به دیجیتال، ابتدا باید موج صوتی را در فواصل زمانی مشخص (براساس نرخ نمونه‌برداری) اندازه‌گیری کنیم. سپس هر اندازه‌گیری را با استفاده از عمق بیت به یک عدد دیجیتال تبدیل می‌کنیم.

### 5. **بازسازی صدا:**
هنگامی که صدا را از یک فایل دیجیتال پخش می‌کنیم، این نمونه‌ها دوباره به یک موج پیوسته تبدیل می‌شوند. هر چه نرخ نمونه‌برداری و عمق بیت بیشتر باشد، کیفیت بازسازی صدا بهتر خواهد بود، زیرا جزئیات بیشتری از موج صوتی اصلی حفظ می‌شود.

### **مثال:**
فرض کن که در حال ضبط یک ساز موسیقی با نرخ نمونه‌برداری 44.1 کیلوهرتز و عمق بیت 16 هستیم. در این حالت، در هر ثانیه 44,100 نمونه از صدا گرفته می‌شود و هر نمونه با دقت 16 بیت ذخیره می‌شود. این اطلاعات به اندازه کافی دقیق است تا صدای طبیعی و با کیفیتی را به ما ارائه دهد.

-----

## Quantizing
نمونه‌ها بدون ارزش اختصاص‌داده‌شده چندان ارزشمند نیستند. بنابراین، پس از اینکه نمونه‌ها گرفته شدند، باید به هر یک ارزش اختصاص داده شود که به این فرایند **کوانتایزیشن (Quantization)** گفته می‌شود. هر Codec تعداد محدودی از مقادیر برای این نمونه‌ها ارائه می‌دهد، همان‌طور که در شکل 5-8 نشان داده شده است. خطوط افقی نمایانگر دامنه و در نتیجه ارزش نمونه است.

![](../image/5-8.png)

همان‌طور که در این تصویر می‌بینیم، برخی از نمونه‌های PCM بر روی خط قرار می‌گیرند و برخی دیگر، که با فلش‌ها مشخص شده‌اند، روی خط قرار نمی‌گیرند. وقتی دامنه نمونه روی یک خط قرار نمی‌گیرد، مقدار واقعی به یکی از مقادیر مجاز تبدیل می‌شود. این امر به وضوح بر دقت سیگنال هنگام بازتولید در گیرنده تأثیر می‌گذارد. اختلاف بین مقادیر کوانتایزشده و واقعی، که در شکل 5-9 نشان داده شده، باعث ایجاد **اعوجاج (distortion)** می‌شود.

![](../image/5-9.png)

این اعوجاج ناشی از خطای کوانتایز کردن غیرمنتظره نیست. این یک عاملی است که در سیستم‌های انتقال صوت پیش‌بینی می‌شود و حتی دارای یک متغیر خاص خود است. به نقل از توصیه‌نامه ITU-T Rec. G.113:

واحد اعوجاج کوانتایز (qdu) در سال 1982 تعریف شد و معادل اعوجاجی است که از یک کدگذاری (A/D) و دیکدگذاری (D/A) توسط یک کدک G.711 متوسط حاصل می‌شود. چنین دستگاهی نسبت سیگنال به اعوجاج 35 دسی‌بل دارد که مطابق [ITU-T O.132] اندازه‌گیری شده است. از نظر مفهومی، تعداد qduهای اختصاص داده‌شده به یک فرآیند PCM خاص باید تنها تأثیر نویز کوانتایز تولید شده توسط این فرآیند بر گفتار را نشان دهد. در عمل، تعداد qduها باید از اندازه‌گیری‌های ذهنی فرآیندهای واقعی یا شبیه‌سازی‌شده تعیین شود، جایی که شرکت‌کنندگان نه تنها به نویز کوانتایز، بلکه به سایر اختلالات تولید شده توسط فرآیند دیجیتال آزمایش‌شده نیز مواجه خواهند شد، از جمله انحرافات از پاسخ فرکانسی ایده‌آل در فیلترهای ضدعلیاسینگ و بازسازی.

مشکل دیگر این خطا این است که هرچه ولوم یا دامنه کمتر باشد، تأثیر اعوجاج بیشتر می‌شود. یک سیگنال با دامنه بالا ممکن است این سطح از خطای کوانتایز را نادیده بگیرد، زیرا این خطا درصد کمتری از کل را تشکیل می‌دهد. کدک‌هایی مانند G.711 نیاز دارند که دقت دامنه در دامنه‌های کمتر بیشتر باشد تا در دامنه‌های بالاتر. به عبارت دیگر، دقت در دامنه‌های کمتر بالاتر است.

برای خلاصه‌کردن، می‌دانیم که نرخ نمونه‌برداری‌های خیلی کم یا خیلی زیاد مشکلاتی ایجاد می‌کند. همچنین می‌دانیم که سطوح نادرست کوانتایز می‌تواند دقت بازسازی سیگنال در گیرنده را تحت تأثیر قرار دهد. حال چه نرخ نمونه‌برداری و سطوح دامنه‌ای مناسب هستند؟ برای مثال، کدک G.711 PCM در هر ثانیه 8000 نمونه ایجاد می‌کند. دلیل این مقدار اغلب به هری نیکوییست نسبت داده می‌شود. فرض این است که برای بازتولید دقیق یک سیگنال، باید آن را با نرخی برابر با دو برابر پهنای باند اصلی نمونه‌برداری کنید. به این ترتیب، تمام تغییرات فرکانسی لحاظ می‌شوند. با اختصاص 4000 هرتز (که در واقع انتقال 3.1 کیلوهرتز استفاده می‌کند) به هر حلقه محلی، نرخ نمونه‌برداری باید 8000 نمونه در ثانیه باشد. به عبارت دیگر، فرکانس نمونه‌برداری 8000 هرتز است.

برای تعیین مقدار کوانتایز، از پرسیدن تعداد مقادیر ممکن که باید اختصاص داده شوند شروع کنید. برای مثال، اگر یک دماسنج فقط بتواند در مقادیر ده‌تایی خوانده شود، چگونه دمای 35 درجه را توصیف می‌کنید؟ از طرف دیگر، اگر تنها به اعداد صحیح اهمیت دهیم، دلیلی وجود ندارد که دماسنج برای یک‌دهم درجه دقت داشته باشد. اگر تعداد مقادیر ممکن خیلی زیاد باشد، پهنای باند مورد نیاز کانال ممکن است از ظرفیت مجاز فراتر رود. برای کدک G.711 PCM، هر نمونه بر اساس هشت بیت یک مقدار می‌گیرد. این به معنای 256 مقدار ممکن است، که به این معناست که یک سیگنال مانند آنچه در شکل 5-8 نشان داده شده است، می‌تواند تا 256 خط رزولوشن داشته باشد.

ظرفیت کانال حاصل ضرب این دو است. برای یک کانال کدگذاری‌شده با G.711 PCM، نرخ داده برابر است با نرخ نمونه‌برداری ضربدر بیت‌های هر نمونه:

8000 نمونه × 8 بیت در هر نمونه = 64000 بیت در ثانیه

کدک G.711 موفق‌ترین کدک از نظر امتیاز نظر میانگین (MOS) و مقاومت در برابر از دست دادن بسته است، اما هزینه‌ای دارد. 64Kbps اختصاص‌داده‌شده به یک کانال صوتی G.711 بالاترین میزان پهنای باند مصرفی توسط یک کدک صوتی در اکثر سیستم‌ها است. این مقدار شاید خیلی زیاد به نظر نرسد، اما وقتی نگرانی در مورد پهنای باند وجود دارد، به ویژه در لینک‌های شبکه گسترده (WAN)، 64Kbps ممکن است برای یک تماس به نظر زیاد بیاید. به همین دلیل، روش‌های دیگری برای فشرده‌سازی بیشتر و کاهش منابع مصرفی توسعه یافته‌اند. با این حال، حتی با انواع مختلف فشرده‌سازی، مدولاسیون کد پالس (PCM) اساس رایج‌ترین کدک‌ها را تشکیل می‌دهد.

## ITU-T G Series Specifications
### کدک‌های محبوب صوت و ویدیو در سری G ITU-T

کدک‌های صوتی و تصویری محبوب در استانداردهای سری G سازمان ITU-T تعریف شده‌اند. به طور کلی، این کدک‌ها به دو نوع تقسیم می‌شوند: **کدک‌های موج (Waveform Codecs)** و **کدک‌های منبع (Source Codecs)**.

#### کدک‌های موج (Waveform Codecs)
کدک‌های موج، موج آنالوگ را نمونه‌برداری و فشرده‌سازی می‌کنند و در حین انجام این کار، کوانتایز نیز انجام می‌دهند. مقادیر کوانتایز شده به گیرنده ارسال و به‌صورت مستقیم به موج اولیه تبدیل می‌شوند. این کدک‌ها اغلب بر اساس مدولاسیون کد پالس (PCM) هستند و به راحتی قابل تشخیص‌اند.

#### کدک‌های منبع (Source Codecs)
کدک‌های منبع، مانند **G.729**، سعی می‌کنند مقداری که داده را توصیف می‌کند (مثل متادیتا) پیدا کرده و به جای داده اصلی ارسال کنند. برای مثال، G.729 نمونه‌های صوتی را برای ضرایب پیش‌بینی‌کننده تحلیل می‌کند و این پارامترها را رمزگذاری و ارسال می‌کند.

### G.711: مدولاسیون کد پالس (PCM) برای فرکانس‌های صوتی
این توصیه‌نامه از سال 1972 وجود دارد. همان‌طور که قبلاً اشاره شد، این کدک تعریف‌کننده کدگذاری صدای PCM با نرخ 64Kbps است و داده‌های صوتی را به فرمت مناسب برای انتقال بر بستر PSTN و PBX تبدیل می‌کند. این کار با نمونه‌برداری از جریان صوتی 8000 بار در ثانیه (هر 125 میکروثانیه یک بار) و اختصاص 8 بیت به هر نمونه انجام می‌شود. این 8 بیت به سه بخش تقسیم می‌شود: یک بیت برای قطبیت، بیت‌های بخش و سطح دامنه. 

در واقع دو نسخه از G.711 وجود دارد: **μLaw (Mu-Law)** و **A-Law**. μLaw در آمریکای شمالی و ژاپن و A-Law در اروپا و سایر نقاط جهان استفاده می‌شود. این کدک برای مواردی که تفاوتی میان نسخه‌ها وجود نداشته باشد، استفاده از A-Law را توصیه می‌کند.

**نکته عملی:** این کدک در تنظیمات سنتی و VoIP بهترین عملکرد کلی را ارائه می‌دهد، حتی در شرایط وجود تأخیر و از دست رفتن بسته‌ها. با این حال، G.711 پهنای باند بیشتری مصرف می‌کند (64Kbps) که معمولاً در شبکه‌های محلی (LAN) مشکلی ایجاد نمی‌کند، اما می‌تواند به شبکه‌های گسترده (WAN) فشار وارد کند.

### G.722: کدگذاری صوتی 7 کیلوهرتزی در 64Kbps
G.722 از تکنیکی به نام **کدگذاری PCM تفاضلی تطبیقی زیرباند (SB-ADPCM)** استفاده می‌کند که در بازه فرکانسی 50 تا 7000 هرتز عمل می‌کند. این کدک سه حالت عملیاتی دارد که به نرخ‌های انتقال 48، 56، و 64Kbps منجر می‌شوند. در این کدک، نرخ نمونه‌برداری 16kHz است و هر نمونه 14 بیت دارد. زیرباندها کانال را به دو بخش پایین (0-4000Hz) و بالا (4000-8000Hz) تقسیم می‌کنند. زیرباند پایین سیگنالی با نرخ 48Kbps تولید می‌کند و زیرباند بالا 16Kbps.

بدون توجه به حالت، کدگذاری پایه G.722 برابر 64Kbps است. پس از مرحله اولیه کدک (رمزگذاری صوتی مبتنی بر PCM)، ممکن است یک یا دو بیت از زیرباند برای کانال داده جانبی با نرخ 8Kbps یا 16Kbps استفاده شود. تعداد بیت‌های اختصاص داده‌شده به این کانال داده به حالت کدک بستگی دارد. G.722 باید قابلیت همکاری با **G.721** (32Kbps) را داشته باشد، و G.725 جزئیات مربوط به کنترل حالت و هماهنگی (Handshake) را فراهم می‌کند.

**نکته عملی:** این کدک به اندازه G.711 عملکرد ندارد، اما نزدیک به آن است. یکی از مزایای واقعی G.722 توانایی آن در **پنهان‌سازی از دست رفتن بسته‌ها (PLC)** است که به پیش‌بینی آنچه نمونه صوتی باید می‌بود، کمک می‌کند.

### G.723.1: کدکننده صوتی دو نرخ برای ارتباطات چندرسانه‌ای

کدک G.723.1 در سال 2006 ارائه شد و از تکنیک‌های فشرده‌سازی صوتی و گفتاری در نرخ‌های بیت بسیار پایین (5.3Kbps و 6.3Kbps) به عنوان بخشی از استانداردهای H.324 استفاده می‌کند:

- **5.3Kbps** – پیش‌بینی خطی تحریک‌شده توسط کد جبری (ACELP)
- **6.3Kbps** – کوانتایزیشن بیشینه احتمال چندپالس (ML-MLQ)

برخلاف اندازه فریم 20 میلی‌ثانیه‌ای G.711، G.723 از فریمی 30 میلی‌ثانیه‌ای به‌علاوه 7.5 میلی‌ثانیه "پیش‌بینی" استفاده می‌کند، که در مجموع باعث ایجاد تأخیری 37.5 میلی‌ثانیه‌ای در تولید فریم می‌شود. داده‌های صوتی ابتدا مطابق G.711 نمونه‌برداری شده و سپس به PCM خطی 16 بیتی که توسط رمزگذار مورد نیاز است تبدیل می‌شود. این رمزگذار بر روی بلوک‌های 240 نمونه‌ای (30 میلی‌ثانیه) کار می‌کند. پیش‌بینی خطی و تخمین گام به‌کار می‌رود تا در نهایت یک "فیلتر وزن‌دهی ادراکی کوتاه‌مدت" ساخته شود، که برای ایجاد "سیگنال گفتاری وزن‌دهی‌شده ادراکی" به‌کار می‌رود. مراحل نهایی بسته به نرخ بیت مورد نظر متفاوت است.

**نکته عملی:** این کدک‌ها با نرخ بیت بسیار پایین عمل می‌کنند و بنابراین فقط در شرایطی که پهنای باند محدود باشد مهم هستند. بنابراین، برای VoIP توصیه نمی‌شوند.

### G.726: کدگذاری PCM تفاضلی تطبیقی در 16، 24، 32 و 40Kbps

این کدک که در سال 1990 تأیید شد، کانال 64Kbps استاندارد را به یکی از سرعت‌های ذکرشده تبدیل می‌کند. هر یک از نرخ‌های داده‌ی فردی کاربردهای مختلفی دارند، از تجهیزات چندپلکسر گرفته تا مودم‌ها. این کدک بر اساس سیگنال تفاضلی عمل می‌کند که از تفریق ورودی تخمینی از سیگنال ورودی واقعی به دست می‌آید. کوانتایزینگ سیگنال تفاضلی بر اساس نرخ بیت مورد نظر است و بین 2 تا 5 رقم دودویی به آن اختصاص می‌دهد. برای مثال، در نرخ 40Kbps، پنج بیت اختصاص می‌یابد: یکی برای قطبیت و چهار بیت برای دامنه.

**نکته عملی:** G.726 عملکرد خوبی ارائه می‌دهد، اگرچه تأخیر پردازشی آن بیشتر از G.711 است. بنابراین، پرسش برای استفاده این است که آیا افزایش تأخیر به کاهش پهنای باند می‌ارزد؟

### G.729: کدگذاری صوتی در 8Kbps با استفاده از ACELP ساختار مزدوج

G.729 و ضمیمه A آن کیفیتی مشابه کدگذاری ADPCM در 32Kbps را فراهم می‌کنند. G.729 یکی از محبوب‌ترین کدک‌ها، بعد از G.711 است. برای مثال، برخی از دستگاه‌های سیسکو از G.729 به عنوان کدک پیش‌فرض استفاده می‌کنند. این کدک که در سال 1996 توسعه یافت، از همان پیش‌بینی خطی که قبلاً توضیح داده شد استفاده می‌کند، اما فریم‌های صوتی آن 10 میلی‌ثانیه‌ای و شامل 80 نمونه هستند. نمونه‌برداری هر 125 میکروثانیه (یک‌هشتم‌هزارم ثانیه) انجام می‌شود. با پیش‌بینی 5 میکروثانیه‌ای، تأخیر کل برای تولید فریم 15 میکروثانیه است.

**نکته عملی:** G.729 در محیط شبکه محلی (LAN) به خوبی عمل می‌کند. اما در لینک‌های WAN، همان چالشی که برای G.726 وجود داشت نیز مطرح است: آیا افزایش تأخیر به صرفه‌جویی در پهنای باند می‌ارزد؟ و اتصال در صورت تغییر کدک‌ها چگونه عمل می‌کند؟

## Codec Selection and Performance
تاریخاً، انتخاب کدک (Codec) بر اساس کیفیت و هزینه انجام می‌شده است. اگر پول و پهنای باند محدودیتی نداشت، سازمان‌ها احتمالاً به‌طور انحصاری از G.711 استفاده می‌کردند. با اینکه این کدک از نظر پهنای باند هزینه‌بر است (G.711 به ۶۴Kbps نیاز دارد)، در شرایطی که مشکلات شبکه‌ای مانند تأخیر (Latency)، از دست رفتن بسته‌ها (Packet Loss)، و تغییرات ناگهانی در زمان انتقال (Jitter) وجود دارد، عملکرد بسیار خوبی دارد. تمامی جریان‌های بلادرنگ (Real-Time Streams) تحت تأثیر این مشکلات قرار می‌گیرند، اما در هر شرایطی، G.711 همیشه در مقایسه با دیگر کدک‌ها عملکرد بهتری دارد.

با این حال، اتصال به خارج هزینه‌بر است و بسیاری از کدک‌ها و الگوریتم‌های فشرده‌سازی که قبلاً به آنها اشاره شده بود، توسعه داده شده‌اند تا کیفیتی مشابه با G.711 PCM را با استفاده از پهنای باند کمتر ارائه دهند. دلیل این امر این است که تماس‌ها با خارج از سازمان از طریق لینک‌هایی با پهنای باند بسیار کمتر عبور می‌کنند، بنابراین انتخاب کدک تأثیر بیشتری بر عملکرد دارد. به‌عنوان مثال، سازمانی که از طریق لینک T-1 به دنیای خارج متصل است، محدود به ۱.۵۴۴Mbps است. با استفاده از G.711، هر تماس یک بیست و چهارم از این ظرفیت را مصرف می‌کند. در حالی که کدکی مانند G.729 نیمی از آن را استفاده می‌کند. البته، در نهایت افزایش فشرده‌سازی به منظور حفظ پهنای باند شروع به تأثیر گذاشتن بر کیفیت تماس می‌کند.

یکی از معیارهای رایج برای اندازه‌گیری کیفیت تماس، میانگین امتیاز نظرخواهی یا MOS است که از توصیه ITU-T P.800 استخراج می‌شود. این مقیاس از ۱ تا ۵ است، که ۵ بهترین حالت ممکن است. در واقع، چندین معیار مختلف ممکن است در محاسبه مقدار MOS نقش داشته باشند، همان‌طور که در جدول ۵-۱ نشان داده شده است.

![](../image/t5-1.png)

امتیازات بین 3.9 تا 4.1 به عنوان کیفیت بسیار خوب در نظر گرفته می‌شوند، در حالی که امتیازات کمتر از 3 به عنوان ضعیف تا غیرقابل قبول تلقی می‌شوند. ایده این است که فردی که به یک تماس گوش می‌دهد، وضوح تماس را ارزیابی کند. نرم‌افزار می‌تواند تلاش کند تا MOS مبتنی بر انسان را شبیه‌سازی کند، اما گاهی بهتر است به سادگی گوشی را برداشته و به تماس گوش دهید. یکی دیگر از معیارهای مرتبط با این موضوع، نمره ارزیابی ادراکی کیفیت گفتار (PESQ) است که در توصیه ITU-T P.862 شرح داده شده است. مقیاس‌بندی PESQ مشابه MOS است. نکته مهم این است که این امتیازات بر اساس تجربه انسانی هستند؛ به عبارتی، فردی به تماس گوش می‌دهد و بر اساس آن تجربه، یک مقدار کیفیت از 1 تا 5 انتخاب می‌کند. این یک اندازه‌گیری عددی مانند تأخیر (Latency) یا تغییرات ناگهانی زمان انتقال (Jitter) نیست. با این حال، بسته‌های نرم‌افزاری مانند **Omnipeek** سعی می‌کنند یک مقدار MOS به تماس‌های VoIP اختصاص دهند.

گاهی گفته می‌شود که اندازه‌گیری‌هایی مانند MOS و PESQ در یک محیط VoIP یا ارتباطات یکپارچه (Unified Communications) ارزش زیادی ندارند، زیرا پهنای باند موجود برای هر اتصال کافی است. سرعت لینک‌های داخلی برای گره‌های شبکه داده معمولاً از 100Mbps تجاوز می‌کند و تقاضای 64Kbps برای یک اتصال حتی 1 درصد از پهنای باند را مصرف نمی‌کند. بنابراین، کدک به‌طور معمول بر عملکرد تماس‌های داخلی تأثیر نمی‌گذارد. اما شاید درست‌تر باشد که بگوییم ما از یک مقیاس MOS غیررسمی بدون اینکه متوجه باشیم استفاده می‌کنیم. هنگامی که تغییراتی در سیستم ایجاد می‌کنیم، گره‌هایی (به ویژه کلاینت‌های نرم‌افزاری) اضافه می‌کنیم، سیستم‌ها را ترکیب می‌کنیم یا مسیرهای جایگزین یا پشتیبان را مقایسه می‌کنیم، طبیعی است که گوشی را برداریم و ببینیم آیا تماس به درستی برقرار است یا خیر. بخشی از این فرآیند، یا حداقل باید باشد، مقایسه کیفیت است. مقیاس MOS می‌تواند برای مقایسه اتصال بین سیستم‌های مختلف یا مسیرهای مختلف استفاده شود. آگاهی از عواملی که بر اتصال تأثیر می‌گذارند (مانند کدک) به مدیر VoIP کمک می‌کند تا مشکلات را شناسایی و رفع کند.

## So, what values can we expect?
مشخص شده است که اگرچه عملکرد کدک‌ها متفاوت است، اما معمولاً اختلاف آنها در مقیاس MOS بیش از ۲۰ تا ۴۰ درصد نیست. سیسکو در کتابی با عنوان **VoIP: An In-depth Analysis** که توسط Cisco Press منتشر شده است، ارزیابی‌ای از بسیاری از این کدک‌ها ارائه کرده است. این ارزیابی با استفاده از مقیاس MOS نشان می‌دهد که تقریباً همه کدک‌های صوتی رایج امتیازی بالاتر از ۳.۶ دارند، اما بهترین کدک‌ها (مانند G.726، G.729 و G.711) بین ۳.۸۵ تا ۴.۱ امتیاز دارند. این نتایج با گزارش‌های دیگر که عملکرد کدک‌ها را بررسی کرده‌اند، همخوانی دارد.

تحلیل دیگری از **Broadcom** با استفاده از مقیاس PESQ انجام شده است و می‌توان آن را در وب‌سایت‌های مربوطه یافت. در این ارزیابی، Broadcom مقادیر تأخیر و عملکرد کدک‌ها هنگام انتقال زبان‌های مختلف را نیز مورد بررسی قرار داده است. باز هم، کدک G.711 عملکرد بسیار خوبی از خود نشان داده است، همانند بسیاری از دیگر کدک‌های استاندارد ITU-T. با این حال، اگر وضوح صدا اهمیت زیادی داشته باشد، ممکن است استفاده از G.723 انتخاب مناسبی نباشد.

## Transcoding
در Transcoding یک فرآیند است که طی آن داده‌های صوتی یا ویدیویی که با یک کدک کدگذاری شده‌اند، تبدیل می‌شوند تا یک دستگاه در انتهای دیگر که از کدک دیگری استفاده می‌کند، بتواند آن را درک کند. به عنوان مثال، اگر یک دستگاه صوتی از کدک G.711 برای کدگذاری داده‌ها استفاده کند، سیستم دیگری که از کدک G.729 استفاده می‌کند، نمی‌تواند مستقیماً آن را دریافت و رمزگشایی کند. بنابراین، دستگاهی مانند یک **Gateway** باید جریان G.711 را به G.729 تبدیل کند. در طی این فرآیند، ممکن است خطاهای ترانسکدینگ یا اعوجاج رخ دهد.

این اعوجاج بسیار شبیه به اعوجاج ناشی از فرآیند کدگذاری است که در بخش‌های قبلی به آن اشاره شد. مشکل اینجاست که پس از اینکه یک کدک داده‌ها را نمونه‌برداری و کمیت‌گذاری می‌کند، به آن‌ها یک مقدار خاص اختصاص می‌دهد. اگر یک کدک دیگر همین فرآیند را انجام دهد، مقادیر متفاوتی خواهد داشت. بنابراین، وقتی این مقادیر برای رمزگشایی به یکدیگر داده می‌شوند، نتیجه لزوماً متفاوت خواهد بود. در نتیجه، داده‌های نمونه ممکن است به‌راحتی توسط سیستم متصل تبدیل نشوند، که به تولید مقادیر مختلفی برای نمونه‌های صوتی منجر می‌شود. اعوجاج یا از دست رفتن کامل نمونه‌های صوتی کیفیت تماس را کاهش می‌دهد.

با توجه به تعداد زیاد کدک‌های موجود، منطقی است فرض کنیم که میزان خطاهای ترانسکدینگ بسته به ترکیب کدک‌ها در دو انتهای تماس متفاوت خواهد بود. در کارگاه اولین نشست تلفنی IP (2000)، Alcatel یک نمودار مقایسه‌ای از چندین کدک ارائه کرد که می‌توان آن را در شکل 5-10 مشاهده کرد.

![](../image/5-10.png)

در تصویر اشاره‌شده، که در آن اعداد بالاتر نشان‌دهنده عملکرد بهتر هستند، می‌توان مشاهده کرد که کدک‌های **G.711**، **G.726** و **G.729** عملکرد بسیار خوبی دارند و افت کیفیت کمی در نتیجه ترانسکدینگ تجربه می‌کنند. اما کدک‌هایی که از نرخ بیت پایین‌تری استفاده می‌کنند و از روش‌های متفاوتی برای کمیت‌گذاری داده‌های صوتی بهره می‌برند، دچار افت کیفیت قابل‌توجهی می‌شوند. نکته جالب این است که حتی نقاط انتهایی که از یک کدک مشابه استفاده می‌کنند نیز ممکن است با مشکلات ترانسکدینگ مواجه شوند، چرا که در فرآیند کدگذاری و کمیت‌گذاری مقداری اعوجاج یا از دست رفتن داده رخ می‌دهد.

در زمینه ویدیو، مشکلات مشابه کدگذاری وجود دارد، اما پیچیدگی‌های بیشتری نیز دارد، مانند فرمت. برای مثال، تصور کنید که سه تصویر از یک صحنه گرفته شده است. یکی از آنها از تغییرات رنگ‌های قرمز، سبز و آبی استفاده می‌کند؛ دیگری از سایان، سرخابی و زرد؛ و سومی از روشنایی، اشباع رنگ و رنگ‌مایه. این رنگ‌ها نزدیک به هم هستند اما یکسان نیستند. هنگام تلاش برای تبدیل از یک مجموعه رنگ به مجموعه دیگر، برخی اطلاعات باید به "نزدیک‌ترین مقدار" تبدیل شوند که منجر به از دست رفتن دقت و کیفیت تصویر اصلی می‌شود.

در یک مثال واقعی، شرکت **Kodak** با مشکل معروف به رنگ زرد "K" مواجه شد. این شرکت زمان و پول زیادی صرف کرد تا اطمینان حاصل کند که مردم در سراسر جهان لوگوی آن را می‌شناسند، اما در نهایت متوجه شد که رنگ زرد لوگو از یک کامپیوتر به کامپیوتر دیگر و از مرورگری به مرورگر دیگر تغییر می‌کند.

به دلیل مشکلاتی که ترانسکدینگ به وجود می‌آورد، بیشتر سازمان‌هایی که درگیر ایجاد یا پیاده‌سازی کدک‌ها هستند (مانند ITU-T) دستورالعمل‌هایی برای ایجاد تست‌هایی جهت ارزیابی عملکرد کدک‌ها، از جمله عملکرد آنها در ترانسکدینگ، ارائه داده‌اند.

## Packet Loss and Packet Loss Concealment (PLC)
از دست رفتن بسته‌ها (Packet Loss) یکی دیگر از مشکلاتی است که می‌تواند تأثیر قابل‌توجهی بر عملکرد کدک‌ها و توانایی گیرنده برای درک انتقال داشته باشد. در کدک‌های مبتنی بر موج (Waveform Codecs)، این مشکل به معنی از دست دادن نمونه‌های کمیت‌گذاری شده است. در کدک‌های منبع (Source Codecs) که به مقادیر عددی داده‌ها و فشرده‌سازی بالا متکی هستند، از دست رفتن بسته‌ها می‌تواند خطاهای شدیدی ایجاد کند. به حدی که در **توصیه G.113** از ITU-T، که قبلاً به آن اشاره شده، یک فاکتور **مقاومت در برابر از دست رفتن بسته‌ها** برای کدک‌ها تعریف شده است، که در **جدول 5-3** نشان داده شده است. مقادیر بالاتر در این فاکتور نشان‌دهنده عملکرد بهتر کدک در مواجهه با از دست رفتن بسته‌ها است.

**VAD** مخفف **Voice Activity Detection** است که تشخیص می‌دهد آیا سیگنال صوتی در حال ارسال است یا خیر.

![](../image/t5-3.png)

اگرچه بسیاری از این مقادیر همچنان در حال بررسی هستند، از جمله حداقل و حداکثر مقادیر، واضح است که **G.711** عملکرد خوبی دارد. همچنین مشخص است که موفقیت کدک‌ها در مواجهه با از دست رفتن بسته‌ها متفاوت است.

به دلیل مشکلات جدی که از دست رفتن بسته‌ها می‌تواند ایجاد کند، کدک‌ها برای کاهش بخشی از این آسیب‌ها به تکنیک‌هایی مانند **پنهان‌سازی از دست رفتن بسته‌ها (Packet Loss Concealment - PLC)** یا **پنهان‌سازی حذف فریم‌ها (Frame Erasure Concealment - FEC)** مجهز شده‌اند. هنگامی که کدک تشخیص می‌دهد بسته‌ای از دست رفته است، می‌تواند چند کار برای پوشش این مشکل انجام دهد. برای مثال، کدک ممکن است به جای داده‌ی گم‌شده همه صفرها را جایگزین کند یا سیگنال جاری را تا صفر کاهش دهد تا بسته بعدی برسد. کدک‌ها همچنین می‌توانند بسته قبلی را تکرار کنند تا شکاف پر شود. با این حال، این روش‌ها به عنوان الگوریتم‌های پنهان‌سازی از دست رفتن بسته در نظر گرفته نمی‌شوند.

الگوریتم‌های PLC تلاش می‌کنند تا تعیین کنند سیگنال چه می‌بایست یا چه می‌توانست باشد اگر بسته از دست نمی‌رفت. نمونه‌ای از این رویکرد در **ضمیمه III کدک G.722** قابل مشاهده است. این افزونه به کدک متکی به **برون‌یابی دوره‌ای موج (PWE)** و تخمین گام برای پر کردن شکاف‌های ناشی از فریم‌های گم‌شده است. تکنیکی که برای کشیدن و فشردن سیگنال در طول محور زمان استفاده می‌شود، **تغییر زمان (Time Warping)** نام دارد. این ضمیمه چنان به خوبی از دست رفتن بسته‌ها را پنهان می‌کند که در واقع از برخی تغییرات بعدی برتری دارد. در واقع، G.722 ادعا می‌کند که در حالی که پیچیدگی یا پردازش با این ضمیمه افزایش می‌یابد، در مواردی که این موضوع مشکل‌ساز نیست، می‌توان آن را بدون افزایش تأخیر نسبت به G.722 استاندارد پیاده‌سازی کرد.

این ما را به مشکلی که در اجرای پنهان‌سازی از دست رفتن بسته‌ها وجود دارد، یعنی **تأخیر**، می‌رساند. PLC تا حدی با درک جریان بسته‌های داده صوتی قبل و بعد از بسته گم‌شده کار می‌کند. این بدان معناست که برای اجرای پنهان‌سازی از دست رفتن بسته‌ها، سطحی از تأخیر اجتناب‌ناپذیر است. بنابراین مشکل اینجاست که برای مقابله با مشکلات ناشی از از دست رفتن بسته‌ها، پنهان‌سازی از دست رفتن بسته‌ها را اجرا می‌کنیم که می‌تواند تأخیر را افزایش دهد.

## What Codec Are You Using
هنگام بروز مشکلات در صدا یا ویدیو، دانستن اینکه دقیقاً کدام کدک استفاده می‌شود می‌تواند بسیار مفید باشد. بررسی تنظیمات یا قابلیت‌ها همیشه اطلاعات کامل را در اختیار ما نمی‌گذارد، به‌ویژه اگر ما کنترل کامل مسیر ارتباطی را نداشته باشیم. خوشبختانه می‌توانیم بسته‌های موجود در جریان صدا یا ویدیو را بررسی کنیم تا این اطلاعات را به دست آوریم.

**پروتکل انتقال بلادرنگ (Real-Time Transport Protocol - RTP)** توسط اکثر سیستم‌های VoIP و کنفرانس ویدیویی برای انتقال داده استفاده می‌شود. در **فصل ۴** این پروتکل به‌صورت مفصل‌تر توضیح داده شده است. همان‌طور که از **شکل ۵-۱۱** مشاهده می‌شود، کدک در سربرگ (Header) بسته‌های RTP مشخص می‌شود. محتوای این بسته با استفاده از کدک **G.711 μ-Law** کدگذاری شده است. با استفاده از این مقدار، نقاط انتهایی می‌دانند که از کدام کدک برای رمزگشایی داده‌ها استفاده کنند. در واقع، هر کسی که این بسته‌ها را ضبط کند، قادر خواهد بود داده‌ها را رمزگشایی کند.

ویدیو نیز به همین شکل شناسایی می‌شود، همان‌طور که در **شکل ۵-۱۲** نشان داده شده است.
